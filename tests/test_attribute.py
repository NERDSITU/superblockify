...
def test_new_edge_attribute_by_function_overwriting_disallowed(source,
                                                               destination):
    ...
    with pytest.raises(ValueError):
        new_edge_attribute_by_function(g_path, __func,
                                       source, destination)


@pytest.mark.parametrize("test_input", [4, -4, 0, 2.0, 10e5])
@pytest.mark.parametrize("graph_len", [2, 3, 20])
def test_new_edge_attribute_by_function_overwriting_allowed(test_input,
                                                            graph_len):
    """Test `new_edge_attribute_by_function` by design."""
    ...
    g_path = nx.path_graph(graph_len)
    nx.set_edge_attributes(g_path, test_input, 'in')
    new_edge_attribute_by_function(g_path, __func, 'in', 'in',

...
def split_up_isolated_edges_directed(graph, sparsified):
    ...
    Also, two parallel edges are considered isolated together, if their geometries
    equal.

    For these a node is inserted at a middle point of the geometry of the edge(s),
    ...
    NotImplementedError
        Parallel edges of degree higher than 2 are not supported.


    Notes
    -----
    The graph is modified in place.
    The new node ids are generated by the function `get_new_node_id`.
    """
    ...
    for u_isol, v_isol in [
        ...
        if rest.degree(u_isol) == 1 and rest.degree(v_isol) == 1:
            edges = [
                (  # single edge with unknown direction and key
